...
function generateTodoObject(id, task, timestamp, author, isCompleted) {
  return {
    id,
    task,
    timestamp,
...
function makeTodo(todoObject) {
  const { id, task, timestamp, isCompleted, author } = todoObject;
  ...
  if (isCompleted) {
    ...
    const deleteButton = document.createElement("button"); // Tombol hapus
    deleteButton.classList.add("trash-button");
    deleteButton.addEventListener("click", function () {
      removeTask(id); // Fungsi untuk menghapus buku yang belum dibaca
    });
